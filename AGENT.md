...
이 프로젝트의 목적은 미국 주식 1분봉 데이터를 이용해 **초단타 매수·매도 타이밍 AI 모델**을 개발하는 것이다.
...
5. 현재 ask와 미래 bid 기반 1·2·3·5분 실행가능 수익 표면 생성
...
11. 날짜 OOF cross-sectional 진입 효용·순위 모델
12. 포지션 상태를 포함한 다음 1분 청산 continuation value 모델
13. OOF 정책 선택과 순차 수익률 백테스트
14. 모델과 전처리 artifact 저장
...
현재 primary V8은 희소한 급등 여부를 직접 분류하지 않고 **진입 후 1·2·3·5분의
실행가능 수익과 같은 시각 종목 간 상대 효용**을 예측한다.

```text
진입 기준: last_ask_t
미래 범위: 정확히 연속된 t+1..t+5
수익 표면: last_bid_[t+h] / last_ask_t - 1 - 추정 왕복 비용, h=1,2,3,5
진입 target: 5분 최대수익, 3분 종가수익, 선행 손실, 도달시간을 합친 연속 효용
청산 target: 보유 1~4분 상태에서 next_bid / current_bid - 1
```

현재와 미래 quote는 유효해야 하고 1분 간격이 끊기면 라벨을 만들지 않는다. Quote는
라벨·포지션 평가·시장가 백테스트에만 사용하며 차트 encoder 입력은 `t`까지의 OHLC
파생 특징으로 제한한다.
...
```text
entry_utility_q10 / entry_utility_q50 / entry_utility_q90
cross_sectional_entry_rank
predicted_net_return_1m / 2m / 3m / 5m
next_minute_return_q10 / q50 / q90
HOLD / SELL
```

분위수 출력은 수익률 단위이며 cross-sectional rank는 같은 판단 시각의 후보 정렬에
사용한다. V5의 `surge_score_3m`과 10분 PnL baseline 출력은 별도 artifact로 유지한다.
...
V8의 수익 표면은 실행가능 ask→bid 수익에서 추정 왕복 비용을 차감한다. 실제 백테스트는
주문금액별 commission, SEC fee, TAF를 다시 정확히 계산한다. V5의 `target_surge_3m`은
예외적으로 수익 라벨이 아니라, 미래 실행 가능 호가가
...
하나라도 있으면 테스트를 실패시킨다. V8 청산 모델의 포지션 상태 수익률은 이미 체결한
포지션의 실시간 평가값이며 차트 encoder 입력과 별도 채널로 관리한다.
...
### Quote Surge V5: 이전 3분 이진 탐지 MLP

이전 비교 모델은 다음 규칙을 사용한다.
...
유효하지 않으므로 V7 분류 점수를 조건으로 하는 Exit AI는 학습하지 않는다. V8 청산
모델은 V7 신호와 무관하게 모든 가상 진입 상태의 연속 continuation value를 학습한다.

### Value Ranking V8: 현재 진입·청산 primary

V8은 최근 15봉 × 12개 OHLC sequence와 현재 시점의 OHLC context 24개를 사용한다.
현재 ask와 미래 bid는 수익 표면 라벨에만, 진입 후 current bid는 포지션 평가 채널에만
사용한다. 거래량·VWAP·quote flow는 encoder feature에 포함하지 않는다.

```text
entry targets: utility q10/q50/q90, cross-sectional rank, 1/2/3/5분 net return
exit target: 다음 1분 executable bid return q10/q50/q90
entry OOF: 7 Train 날짜 중 앞 3일 이후 4개 expanding-date fold
policy selection: OOF만 사용, 최소 20건, OOF 모든 날짜 active
diagnostic test: 7/16~17, policy 선택에 사용하지 않음, non-pristine
```

현재 결과는 다음과 같다.

```text
Entry OOF utility Spearman:       0.204
Entry OOF cross-sectional IC:     0.094
Exit OOF next-return Spearman:    0.083
OOF trades / mean net return:     765 / -0.499%
Test trades / mean net return:    943 / -0.393%
Deployment status:               FAIL_OOF
```

상대 순위와 청산 방향 신호는 약하게 존재하지만 왕복 비용을 넘지 못했다. Test outcome을
보고 threshold를 다시 조정하지 않으며, 새 독립 날짜 확보 전에는 배포 후보가 아니다.
...
## Phase 5.7. 하락봉 직후 3분 급등 이진 탐지 (Historical Baseline)
...
## Phase 5.9. Cross-sectional Value Entry + Continuation Exit (Current Primary)

1. 모든 유효 판단 시점에서 current ask→future bid 1·2·3·5분 수익 표면 생성
2. 최근 15봉 × 12 OHLC sequence와 24 OHLC context 구성
3. 효용 q10/q50/q90, 동일 시각 종목 rank, horizon return multi-task 학습
4. 날짜 expanding OOF의 cross-sectional IC로 epoch 선택
5. 각 가상 진입의 보유 1~4분 상태와 다음 1분 bid return으로 Exit model 학습
6. Exit q50>0 및 q10>-1%일 때 HOLD, 아니면 SELL, 최대 보유 5분
7. OOF에서만 top-k와 효용 분위수 threshold grid 선택
8. OOF 모든 평가 날짜 active, 최소 20건, 최악 날짜 수익률 우선
9. current ask 시장가 매수·future bid 즉시 매도, 별도 fill gate 없음
10. Test는 선택에 사용하지 않고 고정 policy 진단만 수행
...
## 22.9 Value Ranking V8

```text
current ask와 future bid만으로 1/2/3/5분 return surface가 계산되는지 검증
q10 <= q50 <= q90 단조성 검증
Exit target이 current bid→next bid 1분 수익인지 검증
시장가 백테스트에 NO_FILL 또는 fill probability gate가 없는지 검증
OOF policy 선택에 Test outcome과 Test score 분포가 사용되지 않는지 검증
OOF active-session coverage와 최소 거래 수가 적용되는지 검증
```
...
## 23.3 Value Ranking V8

실제 설정은 `configs/value_ranking_strategy.yaml`, 구현은
`src/value_ranking_strategy.py`를 사용한다. 파생 artifact version은
`cross_sectional_value_timing_v8`이다.

## 23.4 Legacy PnL 초기 설정
